...
function Home() {
  ...
  const handleFileDownload = () => {
    ...
    axios.get('http://localhost:5000/api/download', {
      ...
      .then((response) => {
        ...
        link.href = url;
        link.setAttribute('download', `${compressedFilePath.split('/').pop()}`); // Set the desired file name
  ...
  return (
    <Container maxWidth="md" sx={{ mt: 4 }}>
      <Paper elevation={3} sx={{ p: 4, backgroundColor: '#333', color: '#fff' }}>
        <Typography variant="h4" component="h1" gutterBottom>
          Register to upload your files to the cloud!
        </Typography>
        ...
        >
          <input {...getInputProps()} />
          <Typography color="inherit">Drag 'n' drop some files here, or click to select files</Typography>
        </Box>
